(ns crossref.main
  "Kotodama WASM entrypoint for the Crossref clean-room actor (L4) — Clojure port.
  ...
  facts (`crossref.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "crossref")
(def tier "L4")
...
(def entity-specs
  [{:entity "Work"      :plural "works"      :id-prefix "crossref_wor"
    :fields [:doi :title :type :publishedAt]        :required [:doi :title]
    :coerce {}                       :refs {}}
   {:entity "Author"    :plural "authors"    :id-prefix "crossref_aut"
    :fields [:orcid :givenName :familyName]         :required [:orcid :givenName]
    :coerce {}                       :refs {}}
   {:entity "Citation"  :plural "citations"  :id-prefix "crossref_cit"
    :fields [:workId :citedDoi]                     :required [:citedDoi]
    :coerce {}                       :refs {:workId "Work"}}
   {:entity "Standard"  :plural "standards"  :id-prefix "crossref_sta"
    :fields [:designation :title :organization :status] :required [:designation :title]
    :coerce {}                       :refs {}}
   {:entity "Record"    :plural "records"    :id-prefix "crossref_rec"
    :fields [:accession :database :title]           :required [:accession :database]
    :coerce {}                       :refs {}}
   {:entity "Subject"   :plural "subjects"   :id-prefix "crossref_sub"
...
(defn emit-facts
  "EAVT facts for one record: {\"crossref.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "crossref-compat" :tier tier :entities entities} 200])
...
(defn start! [] :crossref-compat/ready)
